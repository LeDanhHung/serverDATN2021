...
export const ResetPassWord = async (req, res, next) => {
    try {
        ...
        if(phone.length <1 || password.length <1 ) {
           return res.status(400).send({ message: "Số Điện Thoại Và Mật Khẩu Không Được Để Trống" });
        } 
        ...
        return res.status(200).json({
            message: 'Cập Nhật Quên Mật Khẩu Thành Công',
            data: newUser
        })
    } catch (error) {
        console.log(error);
...
export const getPhone = async (req, res, next) => {
    ...
    try {
        ...
        if(!user){
            return res.status(404).send({message: 'Không Tìm Thấy Số điện Thoại'})
        }
        res.status(200).json({
            status: 'Tìm Thấy Số Điện Thoại Thành Công',
            data:  user 
        })
    } catch (error) {
        console.log(error);
